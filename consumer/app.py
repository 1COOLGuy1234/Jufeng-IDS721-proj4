...
def names_to_wikipedia(names):
    ...
    LOG.info(f"names to wikipedia done")
    return df
...
def apply_sentiment(df, column="wikipedia_snippit"):
    ...
    LOG.info(f"apply sentiment done")
    return df
...
def write_s3(df, bucket, name):
    ...
    LOG.debug(f"s3 before resource")
    s3_resource = boto3.resource("s3")
    LOG.debug(f"s3 after resource")
...
def lambda_handler(event, context):
    ...
    LOG.info(f"How df looks like: {df.to_dict()}")
    ...
    # Write result to S3
    write_s3(df=df, bucket="fangsentiment-jz", name=names)
